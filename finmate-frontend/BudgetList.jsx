...
export default function BudgetList({ budgets, loading, onDelete }) {
  if (loading) {
    return <p className="font-mono text-bone/65 text-sm">loading budgets…</p>;
  }
  ...
  return (
    <div className="space-y-3">
      {budgets.map((b) => (
        <div key={b.id} className="border border-hairline rounded-md p-4 group">
          ...
          <div className="mt-2 h-1.5 w-full bg-hairline/30 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${b.overBudget ? "bg-signal" : "bg-ledger"}`}
