  ...
  if (loading || !group) {
    return (
      <div className="min-h-screen bg-app flex items-center justify-center">
        <p className="font-mono text-bone/65 text-sm">loading group…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-app">
      ...
      <main className="p-6 max-w-4xl mx-auto">
        <div className="flex items-start justify-between">
          ...
          {group.createdById === user?.id && (
            <button
              ...
              disabled={deleting}
              className="font-mono text-signal/70 hover:text-signal text-xs uppercase tracking-wide border border-signal/30 hover:border-signal rounded-md px-3 py-1.5 transition-colors disabled:opacity-50"
            >
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-[minmax(0,320px)_1fr] gap-6">
          <form onSubmit={handleSubmit(onSubmit)} className="border border-hairline rounded-md p-5 h-fit" noValidate>
            <p className="font-mono text-xs text-bone/65 uppercase tracking-wide mb-4">Add group expense</p>

            <label className="font-mono text-xs text-bone/75 uppercase tracking-wide">Description</label>
            <input
              type="text"
          ...
          <div className="space-y-6">
            <div>
              <p className="font-mono text-xs text-bone/65 uppercase tracking-wide mb-3">Expense history</p>
